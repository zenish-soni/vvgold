var ViewModel = function() {
    ...
    self.recordData = function(){
        ...
        kp.lu_category_id = ko.observable().extend({required:true});
        kp.lu_sub_category_id = ko.observable().extend({required:true});
        kp.name = ko.observable().extend({required:{message:'Enter name' }});
        kp.file = ko.observable().extend({
            required:{
                onlyIf: function() {
                    return kp.id() == 0;
                }
        ...
        kp.lu_category_id.subscribe(function(value){
            self.subCategories([]);
            if(value){
                const categoryDetail = categories.find(category => category.id == value);
                
                if(categoryDetail && categoryDetail.sub_category){
